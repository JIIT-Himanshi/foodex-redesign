		...
		<section className="relative overflow-hidden bg-(--color-bg-primary) flex items-center justify-center lg:min-h-[calc(100vh-72px)] py-16 md:py-20 lg:py-0">
			...
			<div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center max-w-6xl mx-auto w-full px-4 sm:px-6">
				...
				<div className="flex flex-col items-center text-center lg:items-start lg:text-left">
					...
					<h1
						className={`font-display mt-4 text-4xl font-extrabold leading-[1.05] tracking-[-0.03em] text-(--color-text-primary) transition-all duration-700 ease-out sm:text-5xl md:text-6xl lg:text-[4.25rem] ${
							isReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
						}`}
					...
					<p
						className={`font-body mt-6 max-w-xl text-[18px] leading-8 text-(--color-text-secondary) transition-all duration-700 ease-out ${
							isReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
						}`}
					...
					<div
						className={`mt-8 flex flex-col items-center gap-4 transition-all duration-700 ease-out sm:flex-row sm:justify-center lg:justify-start ${
							isReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
						}`}
